...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($user_login_err) && empty($user_password_err)) {
    $sql = "SELECT id, username, password, phnum FROM users WHERE username = ? OR email = ?";
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
      ...
      if (mysqli_stmt_execute($stmt)) {
        ...
        if (mysqli_stmt_num_rows($stmt) == 1) {
          mysqli_stmt_bind_result($stmt, $id, $username, $hashed_password, $phnum);
          ...
          if (mysqli_stmt_fetch($stmt)) {
            if (password_verify($user_password, $hashed_password)) {
              ...
              // Check if phnum is equal to 1, then redirect to admin dashboard
              if ($phnum == 1) {
                echo "<script>" . "window.location.href='./indexa.php'" . "</script>";
                exit;
              } else {
                echo "<script>" . "window.location.href='./shcat.php'" . "</script>";
                exit;
              }
            } else {
